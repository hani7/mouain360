...
def register_view(request):
    ...
    if request.method == 'POST':
        ...
        if not username or not email or not password1 or not password2:
            ...
            # Send Email
            subject = 'Vérification de votre compte Mouin 360'
            html_message = render_to_string('accounts/emails/otp_email.html', {'user': user, 'otp_code': otp_code})
...
def resend_otp_view(request):
    ...
    # Send Email
    subject = 'Nouveau code de vérification - Mouin 360'
    html_message = render_to_string('accounts/emails/otp_email.html', {'user': user, 'otp_code': otp_code})
    plain_message = strip_tags(html_message)
    try:
        send_mail(subject, plain_message, settings.DEFAULT_FROM_EMAIL, [user.email], html_message=html_message, fail_silently=False)
